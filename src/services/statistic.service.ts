...
export const getGeneralStatistic = async (userId: Types.ObjectId): Promise<getGeneralStatisticReturnType | null> => {
  const isUserHasTransactions = await Transaction.countDocuments({ ownerId: userId })
  ...
  const userExpensesThisMonth: expenseIncomeType[] = await Transaction.aggregate([{
    $match: {
      'ownerId': new Types.ObjectId(userId),
      'isExpense': true,
      $expr: {
...
export const getStatisticForRange = async (userId: Types.ObjectId, from: Date, to: Date): Promise<getStatisticForRangeReturnType | null> => {
