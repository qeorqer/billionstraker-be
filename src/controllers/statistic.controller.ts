...
export const getGeneralStatistic: ControllerFunction = async (req, res, next) => {
  try {
...
export const getStatisticForRange: ControllerFunction = async (req, res, next) => {
  try {
    const { id: userId } = req.body.user
    const { from, to } = req.body

    const statistic = await statisticService.getStatisticForRange(userId, from, to)
